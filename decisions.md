...
### 6. Guard Express Server Startup
...
- **Buggy snippet**:
  ...
  ```
- **Problem**: Not Starting the server when needed and can unconditionally meant that importing `server/index.js` from tests or another module also started a second server on the configured port. This caused port conflicts, prevented the server from running properly, and left an unclosed listener during tests.
- **Fix**: Start listening only when `server/index.js` is executed directly. When imported, it now exports the Express app without opening a listener, allowing tests to bind their own temporary port.
